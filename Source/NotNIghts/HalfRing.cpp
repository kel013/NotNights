...
void AHalfRing::SetUpRingObjects()
{
	...
	float CircleCircumference = 2 * Radius * PI;
	...
	for (int x = 0; x < LengthWiseNumberOfSections / 2; x++)
	{
		...
		MeshStringRight.Append(FString::FromInt(2 * x));
	...
	double Degree = 180.0f / CircleNumberOfSections;
	FVector HalfCircleCenter = FVector::UpVector * StraightLength;
	for (int x = 0; x < CircleNumberOfSections; x++)
	{
		FString MeshString = "RingBall";
		MeshString.Append(FString::FromInt(LengthWiseNumberOfSections / 2 + x));
